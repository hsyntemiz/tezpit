...
#BW_LINK=10000000 # 10 Mbps
#SLEEP_PERIOD = 3
SHOW_ADD_FLOW = True
SHOW_ACCESS_TABLE=False
QOS_ENABLED=True # ALL TRAFFIC TYPES BECOME SAME!
Q15_BUF=30000000
Q31_BUF=30000000
Q47_BUF=60000000

CUSTOMERS = {'Customer_A': 'gold', 'Customer_B': 'silver', 'Customer_C': 'bronze'}
FEE = {'gold':(18,24),'silver':(6,10),'bronze':(2,4)}


FEE_PERIOD=86400 #1 day in secs
ALLOWED_DOWN_TIME_RATIO=[0.1,0.01,001,0001,0.00001]
LINK_DOWN_PROB=0.01
# 3 9uptime 86.4 sec a day down time
# 4 9uptime 8.64 sec a day
# 5 9uptime 0.9 sec down time for a day
...
class Shortest_Route(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #self.access_control = kwargs['Access_Control']
        #
        self.mac_to_port = {}
        self.datapaths = {}
        ...
        self.logger.info("&&&&&&&&&& FEE --------------- %s", FEE[CUSTOMERS['Customer_A']][0])
    ...
    def flow_on_link_init(self):
        ...
        #self.logger.info("xx LINK: %s", self.flows_on_link.keys())
  ...
  # Geri don buraya!!!!
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            ...
            if True:
                ...
                if not myflow in self.flowPathMap.keys():
                    #self.logger.info("\n XXXXXXXXXXX my flow in packet handler: %s XXXXXXXXXXXXXX", myflow)

                    #QoS Check
                    policy = self.qos_check(msg)
                    #self.logger.info("\n XXXXXXXXXXX QoS CHECK - my flow in packet handler: %s XXXXXXXXXXXXXX", policy)
    ...
    #########################################################
    # QoS ISSUES     ########################################
    #self.access_control(myflow, policy, path_, path_of_bw)
    def access_control(self,myflow, policy, path_, path_of_bw):
        self.logger.info("+++++++++++++++++ ACCESS CONTROL myflow: %s", myflow)
        self.logger.info("+++++++++++++++++ ACCESS CONTROL Policy: %s", policy)
        self.logger.info("+++++++++++++++++ ACCESS CONTROL Path: %s", path_)


        #Get fee for flow
        customer=policy[-5]
        fee=int(policy[-1])*int( FEE[CUSTOMERS[customer]][0] )
        self.logger.info("+++++++++++++++++ ACCESS CONTROL # Get fee: %s", fee)

        #Calc Probability of availability of DT_p > ADT_p)
        Prob=0.01

        #Get penalty if flow not satisfied
        penalty= fee


        # Estimate profit for Flow Rquest -
        #  E_f = fee - Pr(DT>ADT)*Penalty_for_flow
        Est_profit= fee - Prob*penalty


        #Calculate COST for flow (direct+ indirect)
        # calculate direct cost

        #calculate indirect cost
        #C_r = (bandwidth)*(1/(1-a_p))*(demand weigth sum of links in chosen path)
        #Her accespt edip install ettigim flow icin weigthed sum islerini guncelle!
    ...
    def send_qos_req(self,dp_first, dp_last, flow_info, routing_mode, policy):
        ...
        if policy:
            ...
            customer=None
        #self.logger.info("XXXXXXXXXXXtuple myflow: %s", myflow)
        ...
        try:
            ...
            #self.logger.info("===%s==%s=================%s===================================",dp_first,dp_last,self.datapaths)
            #self.logger.info("===cccustomer: %s===================================",self.flowPathMap[myflow][-1])
            ...
            #Yeni kayit aciyorum 0 version numarali
                ...
                version_=0
                lock_flag_=False
                self.flowPathMap[myflow]=(0,version_,lock_flag_,None,customer)
            ...
            if datapath_first.id == datapath_last.id:
                ...
                if routing_mode == None :
                    ...
                    self.logger.info("POLICY # %s #!!!! ESTABLISHED##### Path is: %s -Path of bw:%s - POLICY:%s ", routing_mode, path_,path_of_bw,policy)
            ...
            self.logger.info("== send_qos_req exception 1: %s==========flow:%s",err_msg,myflow)
        ...
        try:
            if path_:
                ...
                self.flowPathMap[myflow] = (path_, version_, lock_flag_, policy,customer)

                self.add_flow_to_db(myflow)
                self.logger.info(
                    "################## Flow Number : =  %s",len(self.flowPathMap.keys()))
                ...
                self.logger.info("################### flow_path_map => no possible path for flow:%s ",myflow)
            ...
            self.logger.info("== send_qos_req exception 2: %s==========flow:%s",err_msg,myflow)
    ...
    def periodic_qos_fail_detect(self):
        ...
        for flow in self.flowPathMap.keys():
            path,version,flag,policy,customer=self.flowPathMap[flow]
            ...
            if routing_mode=='bw':
                ...
                if (dp_first,flow) in self.flowStats.keys():
                    ...
                    if  rate and rate > policy[-1]*1024:
                        ...
                        #Burayi comment out yaptim simdilik
                        #self.send_qos_req(dp_first,dp_last,dict_flow_info,routing_mode,policy,customer)
    ...
    def qos_check(self, msg):
        ...
        #IF QoS disabled all traffic become same! Make Flag True to enable QoS (Queue vs vs)
        if not QOS_ENABLED:
            return None
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            ...
            sql_request="select *,case WHEN '%s' LIKE CONCAT(src_ip,'%%') then 1 else 0 end + case src_mac when '%s' then 1 else 0 end + case WHEN '%s' LIKE CONCAT(dst_ip,'%%') then 1 else 0 end + case dst_mac when '%s' then 1 else 0 end + case ip_proto when '%s' then 1 else 0 end + case src_port when '%s' then 1 else 0 end + case  when ('%s' like dst_port) then 1 else 0 end as matches from policies order by matches desc"  % (
                src_ip, src_mac, dst_ip, dst_mac, protocol, src_port, dst_port)
            ...
            if cur.rowcount > 0:
                ...
                if int(number_empty_field)+int(match_field_number)<7:
                    self.logger.info("DB_SCRIPT------ not not not EXACT MATCH !!! empty field:%s match_field:%s",number_empty_field,match_field_number)
                    return None

                self.logger.info("DB_SCRIPT------ EXACT MATCH !!! -- %s",ihave_[-6])
    ...
    #                                self.update_flow_stat_db(flow_tuple,rate,uptime)
    def update_flow_stat_db(self,flow,rate,uptime):
        ...
        if len(flow)==4:
            ...
            cur = self.db.cursor()
            #mysql> UPDATE activeflows SET  bandwidth='779.1304347' and age='12.12121' WHERE src_ip='10.0.1.2' and dst_ip='10.0.3.2' and ip_proto='1' and vlan='0';

            sql_request = "UPDATE activeflows SET bandwidth='%s', path='%s', age='%s' WHERE src_ip='%s' and src_mac='%s' and dst_ip='%s' and dst_mac='%s' and ip_proto='%s' and vlan='%s' " % (
                        float(rate*0.001),path,uptime,src_ip, src_mac, dst_ip ,dst_mac, ip_proto,vlan)
            ...
            cur = self.db.cursor()
            sql_request = "UPDATE activeflows SET bandwidth='%s', path='%s', age='%s' WHERE src_ip='%s' and src_mac='%s' and dst_ip='%s' and dst_mac='%s' and ip_proto='%s' and src_port='%s' and dst_port='%s'and vlan='%s' " % (
                        float(rate * 0.001),path,uptime,src_ip, src_mac, dst_ip ,dst_mac, ip_proto,src_port,dst_port,vlan)
            #sql_request = "UPDATE activeflows SET age='%s'and bandwidth='%s' WHERE ( src_ip='%s', src_mac='%s', dst_ip='%s' ,dst_mac='%s', ip_proto='%s',src_port='%s',dst_port='%s',vlan='%s')" % (
            #            uptime, rate,src_ip, src_mac, dst_ip ,dst_mac, ip_proto,src_port,dst_port,vlan)
    ...
    def path_bw_calculate_usage(self, path, mode):
    ...
    def sort_paths_by_bw_used(self, src_sw, dst_sw, mode=None):
        ...
        for pathik in possible_paths:
            ...
            #self.logger.info("SORTED PATHS REGARDING -(used_bw) %s---- pathik:%s-----mode:%s", used_bw,pathik,mode)
            used_bw22 = self.path_bw_calculate_usage(pathik,'link')
            #self.logger.info("SssORTED PATHS REGARDING -(used_bw) %s---- pathik:%s-----mode:%s", used_bw22,pathik,'link')
        ...
        self.find_path(src_sw,dst_sw,mode)

        return route_dict

    def sort_paths_by_bw_used_mult(self, src_sw, dst_sw, mode=None):
    ...
    def path_fuzzy_calc(self,path,mode=None):
        self.logger.info("PATH_FUZZZY_CALC FOR %s",path)
    #path bw calculation with PORT  QUEUE
        ...
        bandwidth = ctrl.Antecedent(np.arange(0, 10000000, 1), 'bandwidth')
        ...
        tipping.input['quality'] = len(path)
        tipping.input['service'] = 9.8
        ...
        """
        self.logger.info("TIP:%s",tipping.output['tip'])
    ...
    def find_arp(self, ip_addr):
        ...
        self.logger.info("XXXXXXXXXXX find arp %s XXXXXXXXXXXXX", ip_addr)
    ...
    def flow_removed_handler(self, ev):
        ...
        try:
            #Unlocked path
            if flow_tuple in self.flowPathMap and not self.flowPathMap[flow_tuple][2]:
            ...
            elif flow_tuple not in self.flowPathMap:
                pass
                #self.logger.info("Flow_Removed => THIS FLOW HAS ALREADY BEEN DELETED! %s ",flow_tuple)

            elif flow_tuple in self.flowPathMap and self.flowPathMap[flow_tuple][2]:
                self.logger.info("FLOW LOCKED 'cannot deleted' => flow: %s ",flow_tuple)


        except:
            err_msg= sys.exc_info()[0]

            self.logger.info("<except: %s> Flow_Removed => THIS FLOW HAS ALREADY BEEN DELETED! %s ", err_msg,flow_tuple)
                    ...
                    if (dp.id,flow_tuple) in self.pre_flowStats: # there is prev flow entry
                        ...
                        if not period:
                            ...
                            if flow_tuple in self.flowPathMap.keys():
                                path_tmp = self.flowPathMap[flow_tuple][0]
                                #self.logger.info('********destination sw-flowstat jobs %s %s',dp.id, path_tmp[-1])

                                if dp.id == path_tmp[0]:

                                    #self.logger.info('********destination sw-flowstat jobsxxxxxxx %s', path_tmp[-1])
                                    self.update_flow_stat_db(flow_tuple,rate,uptime)
    ...
    def link_down_action_handler(self,ev):
        ...
        if status=='down':
            ...
            if src_dp in NETQX.neighbors(dst_dp):
                ...
                NETQX.remove_edge(dst_dp,src_dp)

                #self.logger.info('event: NETQX REMOVE EDGE %s-%s DOWN --- %s ', src_dp,dst_dp, NETQX.edge)
                self.logger.info('event: NETQX REMOVE EDGE %s-%s DOWN --- ', src_dp, dst_dp)
            ...
            #self.logger.info('event: NETQX UPDATED %s !!!! ', NETQX.edge)
            self.logger.info('event: NETQX UPDATED !!! ')

    def check_down_link_usage(self,fail_link):


        for flow in self.flowPathMap.keys():
            current_path=self.flowPathMap[flow][0]
            result=self.seq_in_seq(fail_link,current_path)
            self.logger.info('event: LINK IS %s ==== PATH:%s !!!! RESULT:%s ',fail_link,current_path,result)
            if not result == -1:
                self.logger.info('Problem! - current path--- %s', current_path)


                (path_, version_, lock_flag_, policy,customer)= self.flowPathMap[flow]
                dict_flow_info=self.tuple_to_dict(flow)
                routing_mode = self.qos_policy_check(policy)
                dp_first,dp_last = path_[0],path_[-1]
                #customer=self.flowPathMap[dict_flow_info][-1]
                self.send_qos_req(dp_first, dp_last, dict_flow_info, routing_mode, policy)

            else:
                self.logger.info('NO_Problem! - fail link is not in current path:%s', current_path)
