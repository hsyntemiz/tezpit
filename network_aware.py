...
class Network_Aware(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        #self.switches_.append(datapath)
        #self.logger.info("XXXXXXXXXXXXX   -  %s", self.switches_)
    ...
    def get_graph(self, link_list, opt2=None):
        ...
        cur = self.db.cursor()
        self.logger.info("vvvvv self.switches: %s=%s ----------%s", self.switches,self.datapaths,self.swCount)
        for src in self.switches:
            ...
            for dst in self.switches:
                ...
                if src == dst:
                    ...
                    self.logger.info('NETQX edge ekledik.')
    ...
    def get_topology(self, ev):
        ...
        #self.logger.info("&&&&&&&&&&&&&&&&&&&&&&&&&&&: GET TOPOLOGY in ICINDEyiIIIIIIIIIIIIM switch_list: %s== %s", switch_list[0],self.topology_api_app)

        self.create_port_map(switch_list)
        self.switches = self.switch_port_table.keys()
        #self.logger.info("&&&&&&&&&&&&&&&&&&&&&&&&&&&: GET TOPOLOGY in ICINDEyiIIIIIIIIIIIIM self.switches: %s", self.switches)
        ...
        #self.logger.info("&&&&&&&&&&&&&&&&&&&&&&&&&&&: GET TOPOLOGY in ICINDEyiIIIIIIIIIIIIM")
